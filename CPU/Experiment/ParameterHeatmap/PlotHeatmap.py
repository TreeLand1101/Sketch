...
pivot_f1 = df.pivot(index="MemoryRatio", columns="ForwardRatio", values="F1score")
pivot_precision = df.pivot(index="MemoryRatio", columns="ForwardRatio", values="Precision")
pivot_recall = df.pivot(index="MemoryRatio", columns="ForwardRatio", values="Recall")
pivot_insert = df.pivot(index="MemoryRatio", columns="ForwardRatio", values="InsertTime")
pivot_query = df.pivot(index="MemoryRatio", columns="ForwardRatio", values="QueryTime")

plt.figure(figsize=(8,6))
sns.heatmap(pivot_f1, annot=True, cmap="YlOrRd")
plt.title("F1score Heatmap")
plt.xlabel("ForwardThreshold (ForwardRatio)")
