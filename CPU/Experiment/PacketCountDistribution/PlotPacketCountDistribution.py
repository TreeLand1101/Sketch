...
def plot_frequency_cdf_log(file_path, title):
    frequencies = np.loadtxt(file_path)
    ...
    output_file = file_path.replace('.txt', '') + "_frequency_cdf_log.png"
    plt.savefig(output_file)
    plt.close()
    print(f"saved to {output_file}")

def plot_frequency_cdf_linear(file_path, title):
    frequencies = np.loadtxt(file_path)
    sorted_freq = np.sort(frequencies)
    cdf = np.arange(1, len(sorted_freq) + 1) / len(sorted_freq)
    ...
    output_file = file_path.replace('.txt', '') + "_frequency_cdf_linear.png"
    plt.savefig(output_file)
    plt.close()
    print(f"saved to {output_file}")
...
plot_frequency_cdf_log(prefix + "_all.txt", "Frequency CDF for All Flows")
plot_frequency_cdf_log(prefix + "_filtered.txt", "Frequency CDF for Filtered Flows")

plot_frequency_cdf_linear(prefix + "_all.txt", "Frequency CDF for All Flows")
plot_frequency_cdf_linear(prefix + "_filtered.txt", "Frequency CDF for Filtered Flows")
